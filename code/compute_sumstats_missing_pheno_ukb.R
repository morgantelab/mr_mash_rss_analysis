...
compute_univariate_sumstats_bigsnp <- function(geno_obj, Y, chr=0, standardize=FALSE, 
  ...
  linreg <- function(i, X, Y, cols){
    ...
    whichNotNa <- which(!is.na(Y[, i]))
    
    fit <- bigstatsr::big_univLinReg(X=X, y=Y[whichNotNa, i], ind.col=cols, ind.train=whichNotNa, ncores = 1)
    bhat <- fit$estim
    shat <- fit$std.err
    
    return(list(bhat=bhat, shat=shat, n=length(whichNotNa)))
  }
  ...
  a1freq <- (statz$sum/nrow(X))/2
  alleles_df <- data.frame(chr=chr, bp=geno_obj$map$physical.pos[inds], a1=geno_obj$map$allele1[inds], a2=geno_obj$map$allele2[inds], freq=a1freq)
  ...
  return(list(Bhat=Bhat, Shat=Shat, alleles=alleles_df, n=N, ntot=nrow(Y)))
}
