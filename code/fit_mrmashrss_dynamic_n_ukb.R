...
parser <- add_option(parser, c("--n"), type="character")
...
###Compute n
if(N=="total"){
  n=univ_sumstats$ntot
} else if(N=="mean"){
  n=round(mean(univ_sumstats$n))
} else if(N=="median"){
  n=median(univ_sumstats$n)
} else if(N=="min"){
  n=min(univ_sumstats$n)
} else if(N=="max"){
  n=max(univ_sumstats$n)
}
