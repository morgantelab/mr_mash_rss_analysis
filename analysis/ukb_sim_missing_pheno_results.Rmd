...
were assigned to 20% of the cells of the phenotype matrix.
...
The results show that *mr.mash.rss* and *mvBayesC* (both versions) perform similarly and 
better than the univariate methods. Having missing values violates an assumption of *mr.mash.rss*.
This results in the method achieving lower accuracy than without missing values. However,
it is still competitive with the other actual multivariate methods and is better than the 
univariate methods. *wMT-SBLUP* does poorly as its infinitesimal architecture 
assumption is not well-suited for the true genetic architecture of the traits.
...
<!-- methodz <- c("mr_mash_rss", "mvbayesC_rest", "ldpred2_auto", "bayesR") -->
