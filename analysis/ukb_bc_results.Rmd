...
* _mr.mash.rss_ per-chromosome, as above, but posterior mean of regression coeffcients initialized using the 
estimates from *LDpred2*, and using finemapped variants (via SuSiE-RSS) to compute the data-driven covariance
matrices. NB this is a multivariate method.
...
perc_change_bayesR_fm_prior
```

<!-- The overall percentage change in $R^2$ of *mr.mash.rss init* over *LDpred2* across traits is `r mean(perc_change)`. -->

The overall percentage change in $R^2$ of *mr.mash.rss init prior* over *BayesR* across traits is `r mean(perc_change_bayesR_fm_prior)`.
...
<!-- The results show that *mr.mash.rss init* improves accuracy over *LDpred2* more substantially with a smaller -->
