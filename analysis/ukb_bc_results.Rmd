...
* _mr.mash.rss_ per-chromosome, as above, but posterior mean of regression coeffcients initialized using the 
estimates from *LDpred2*. NB this is a multivariate method.
...
r2 <- c(as.vector(dat_mrmash), as.vector(dat_mrmash_init), as.vector(dat_ldpred2))
method <- rep(c("mr_mash_rss", "mr_mash_rss_init", "ldpred2_auto"), each=80)
trait <- rep(colnames(pheno), each=5)

res <- data.frame(method, trait, r2)

res <- res[which(res$method %in% c("ldpred2_auto", "mr_mash_rss", "mr_mash_rss_init")), ]

res <- transform(res,
                 method=factor(method, levels=c("ldpred2_auto", "mr_mash_rss", "mr_mash_rss_init"),
                               labels=c("LDpred2", "mr.mash.rss", "mr.mash.rss init")),
                 trait=factor(trait))
...
The overall percentage change in $R^2$ of *mr.mash.rss init* over *LDpred2* across traits is `r mean(perc_change)`.

The results show that *mr.mash.rss init* improves accuracy over *LDpred2* for all but two traits. However,
in those two traits the performance is very similar. In general, the improvement in performance is in line with
