...
* [BayesR](https://www.nature.com/articles/s41467-019-12653-0) per-chromosome, with 1000 iterations 
(after 500 burn-in iterations), $h^2$ initialized as 0.5/22 and other default parameters. NB this is 
a univariate method. We used the implementation in the *qgg* R package.
...
genome-wide from further analyses since it is more computationally intensive. *BayesR* gives
unreasonably low prediction accuracy, which hints at issues with the default parameters and/or
this particular implementation.

```{r equal effects filt, fig.height=10, fig.width=13}
scenarioz <- "equal_effects_indep_resid"
...
from the data and, reassuringly, performs similarly to *LDpred2 auto*. Again, **BayesR* gives
unreasonably low prediction accuracy, which hints at issues with the default parameters and/or
this particular implementation.

```{r mostly null filt, fig.height=10, fig.width=13}
scenarioz <- "trait_1_only_effects_indep_resid"
methodz <- c("mr_mash_rss", "ldpred2_auto")
...
the smaller $h^2_g$ (harder scenario for univariate methods). Again, *BayesR* gives
unreasonably low prediction accuracy, which hints at issues with the default parameters and/or
this particular implementation.

```{r shared in subgroups filt, fig.height=10, fig.width=13}
scenarioz <- "blocks_shared_effects_indep_resid"
methodz <- c("mr_mash_rss", "ldpred2_auto")
...
p_blocks_shared_filt <- ggplot(res, aes(x = trait, y = score, fill = method)) +
  geom_boxplot(color = "black", outlier.size = 1, width = 0.85) +
  stat_summary(fun=mean, geom="point", shape=23,
               position = position_dodge2(width = 0.87,   
                                          preserve = "single")) +
...
per-trait genomic heritability was set to 0.2.
...
*mr.mash.rss* clearly does better than *LDpred2 auto*. Again, *BayesR* gives unreasonably low 
prediction accuracy, which hints at issues with the default parameters and/or this particular 
implementation.

```{r equal effects low pve filt, fig.height=10, fig.width=13}
scenarioz <- "equal_effects_low_pve_indep_resid"
methodz <- c("mr_mash_rss", "ldpred2_auto")
...
and covariance matrix that achieves a per-trait variance of 1 and a correlation across traits of 1.
This implies that the effects of the causal variants are equal across responses. In addition, the 
number of causal variants was set to 50,000.
...
In this scenario, we expect the accuracy to be lower because of the much larger number of causal, each 
explaining a much lower proportion of the total $h^2_g=0.5$. Multivariate methods can borrow
information across traits (if effects are shared) and improve accuracy. The results show that 
*mr.mash.rss* clearly does better than *LDpred2 auto* in this scenario too.
...
show that *mr.mash.rss* clearly does better than *LDpred2 auto*, but the improvement compared to the
scenario with 5 traits is small ($mean(\frac{mr.mash.rss R^2}{LDpred2 auto R^2}) = 1.1157$ vs 
$mean(\frac{mr.mash.rss R^2}{LDpred2 auto R^2}) = 1.1057$). Again, *BayesR* gives unreasonably low 
prediction accuracy, which hints at issues with the default parameters and/or this particular 
implementation.

```{r equal effects 10 traits filt, fig.height=10, fig.width=13}
scenarioz <- "equal_effects_10traits_indep_resid"
methodz <- c("mr_mash_rss", "ldpred2_auto")
